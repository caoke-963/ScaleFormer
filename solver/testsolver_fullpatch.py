...
class Testsolver(BaseSolver):
    ...
    def test(self):
        ...
        for i, batch in enumerate(self.data_loader):
            ...
            if self.cuda:
                ...
                print("ms-size = ", ms_image.size())
                print("lms_image-size = ", lms_image.size())
                print("pan_image-size = ", pan_image.size())
                print("bms_image-size = ", bms_image.size())



            with torch.no_grad():
                lms_h, lms_w = lms_image.shape[2:]
                bms_h, bms_w = bms_image.shape[2:]
                pan_h, pan_w = pan_image.shape[2:]

                lms_patch_h, lms_patch_w = lms_h // 2, lms_w // 2
                bms_patch_h, bms_patch_w = bms_h // 2, bms_w // 2
                pan_patch_h, pan_patch_w = pan_h // 2, pan_w // 2

                patches_lms = [
                    lms_image[:, :, :lms_patch_h, :lms_patch_w],
                    lms_image[:, :, :lms_patch_h, lms_patch_w:],
                    lms_image[:, :, lms_patch_h:, :lms_patch_w],
                    lms_image[:, :, lms_patch_h:, lms_patch_w:],
                ]
                patches_bms = [
                    bms_image[:, :, :bms_patch_h, :bms_patch_w],
                    bms_image[:, :, :bms_patch_h, bms_patch_w:],
                    bms_image[:, :, bms_patch_h:, :bms_patch_w],
                    bms_image[:, :, bms_patch_h:, bms_patch_w:],
                ]
                patches_pan = [
                    pan_image[:, :, :pan_patch_h, :pan_patch_w],
                    pan_image[:, :, :pan_patch_h, pan_patch_w:],
                    pan_image[:, :, pan_patch_h:, :pan_patch_w],
                    pan_image[:, :, pan_patch_h:, pan_patch_w:],
                ]
                ...
                for patch_lms, patch_bms, patch_pan in zip(patches_lms, patches_bms, patches_pan):
                    pred_patch = self.model(patch_lms, patch_bms, patch_pan)
                    model_outputs.append(pred_patch)

                # 合并patches
                top = torch.cat(model_outputs[:2], dim=3)  # 上部两个patch横向拼接
                bottom = torch.cat(model_outputs[2:], dim=3)  # 下部两个patch横向拼接
                prediction = torch.cat([top, bottom], dim=2)  # 上下拼接
    ...
    def save_img(self, img, img_name, mode):
        ...
        save_img = np.uint8(save_img * 255).astype('uint8')  # 归一化到 0-255 范围
        ...
        if mode == 'L':
            save_img = save_img[:, :, 0] if save_img.ndim == 3 else save_img
            tiff.imsave(save_fn, save_img)
        elif mode == 'RGB':
            tiff.imsave(save_fn, save_img, photometric='rgb')
        elif mode == 'CMYK':
            if save_img.shape[2] != 4:
                raise ValueError(f"Image shape mismatch for mode {mode}. Expected 4 channels (CMYK), got {save_img.shape[2]}.")
            tiff.imsave(save_fn, save_img)
        else:

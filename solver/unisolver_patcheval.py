    ...
    def eval(self):
        with tqdm(total=len(self.test_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.test_loader, 1):
                ...
                with torch.no_grad():
                    # 获取图像的高度和宽度
                    lms_height, lms_width = lms_image.shape[2], lms_image.shape[3]
                    bms_height, bms_width = bms_image.shape[2], bms_image.shape[3]
                    pan_height, pan_width = pan_image.shape[2], pan_image.shape[3]

                    # 计算patch的尺寸
                    lms_patch_size_h, lms_patch_size_w = lms_height // 2, lms_width // 2
                    bms_patch_size_h, bms_patch_size_w = bms_height // 2, bms_width // 2
                    pan_patch_size_h, pan_patch_size_w = pan_height // 2, pan_width // 2

                    batch_psnr, batch_ssim, batch_qnr, batch_D_lambda, batch_D_s = [], [], [], [], []
                    
                    # 处理每个batch中的每个图像
                    for c in range(lms_image.shape[0]):
                        # 提取四个patch进行处理
                        patches_lms = [
                            lms_image[c, :, :lms_patch_size_h, :lms_patch_size_w],           # 左上
                            lms_image[c, :, :lms_patch_size_h, lms_patch_size_w:],           # 右上
                            lms_image[c, :, lms_patch_size_h:, :lms_patch_size_w],           # 左下
                            lms_image[c, :, lms_patch_size_h:, lms_patch_size_w:]            # 右下
                        ]
                        patches_bms = [
                            bms_image[c, :, :bms_patch_size_h, :bms_patch_size_w],           # 左上
                            bms_image[c, :, :bms_patch_size_h, bms_patch_size_w:],           # 右上
                            bms_image[c, :, bms_patch_size_h:, :bms_patch_size_w],           # 左下
                            bms_image[c, :, bms_patch_size_h:, bms_patch_size_w:]            # 右下
                        ]
                        patches_pan = [
                            pan_image[c, :, :pan_patch_size_h, :pan_patch_size_w],           # 左上
                            pan_image[c, :, :pan_patch_size_h, pan_patch_size_w:],           # 右上
                            pan_image[c, :, pan_patch_size_h:, :pan_patch_size_w],           # 左下
                            pan_image[c, :, pan_patch_size_h:, pan_patch_size_w:]            # 右下
                        ]
                        
                        # 对每个patch进行推理
                        model_outputs = []
                        for patch_lms, patch_bms, patch_pan in zip(patches_lms, patches_bms, patches_pan):
                            patch_lms = patch_lms.unsqueeze(0)  # 添加batch维度
                            patch_bms = patch_bms.unsqueeze(0)
                            patch_pan = patch_pan.unsqueeze(0)

                            # 推理每个patch
                            fake_patch = self.model(patch_lms, patch_bms, patch_pan)
                            model_outputs.append(fake_patch)

                        # 合并patch的输出结果
                        fake_img = torch.cat([torch.cat(model_outputs[:2], dim=3), torch.cat(model_outputs[2:], dim=3)], dim=2)
                        loss = self.loss(fake_img, ms_image)

                        # 计算合并后图像的损失
                        fake_img = fake_img.squeeze(0).cpu().numpy().transpose((1, 2, 0)) * 255
                        ground_truth = ms_image[c, ...].cpu().numpy().transpose((1, 2, 0)) * 255

                        # 计算PSNR、SSIM等指标
                        psnr = cpsnr(fake_img, ground_truth)
                        ssim = cssim(fake_img, ground_truth, 255)
                    ...
                    # 更新进度条显示
                    # t1.set_postfix_str('n:Batch loss: {:.4f}, PSNR: {:.4f}, SSIM: {:.4f}, QNR: {:.4F} DS: {:.4f}, D_L: {:.4F}'.format(
                    #     loss.item(), avg_psnr, avg_ssim, avg_qnr, avg_d_s, avg_d_lambda))  # 这里的loss需要根据实际计算的loss来更新
                    
                    t1.set_postfix_str('n:Batch loss: {:.4f}, PSNR: {:.4f}, SSIM: {:.4f}'.format(
                        loss.item(), avg_psnr, avg_ssim))  # 这里的loss需要根据实际计算的loss来更新
                    t1.update()
            ...
            # 保存配置
            # save_config(self.log_name, 'Val Epoch {}: PSNR={:.4f}, SSIM={:.6f}, QNR={:.4f}, DS:{:.4f}, D_L:{:.4F}'.format(
            #     self.epoch, self.records['PSNR'][-1], self.records['SSIM'][-1], self.records['QNR'][-1], self.records['D_s'][-1], self.records['D_lamda'][-1]))

            save_config(self.log_name, 'Val Epoch {}: PSNR={:.4f}, SSIM={:.6f}'.format(
